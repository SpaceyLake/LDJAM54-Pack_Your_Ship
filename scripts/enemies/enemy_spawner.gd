...
func _on_spawn_timer_timeout():
	...
	goal.x = position.x + randi_range(96, int(get_viewport().get_visible_rect().size.x/2.0) - 96 - position.x)
	if position.y > spaceship.global_position.y:
		goal.y = randi_range(int(spaceship.global_position.y) - 90, int(get_viewport().get_visible_rect().size.y/2.0) + 96)
	else:
		goal.y = randi_range(int(-get_viewport().get_visible_rect().size.y/2.0) - 96, int(spaceship.global_position.y) + 90)
